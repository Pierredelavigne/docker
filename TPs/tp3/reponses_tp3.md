...
docker compose up -d
[+] up 33/33
 ✔ Image postgres:15-alpine Pulled                                            8.6s
 ✔ Image adminer:latest     Pulled                                            8.7s
 ✔ Network tp3_default      Created                                           0.1s
 ✔ Volume tp3_db_data       Created                                           0.0s
 ✔ Container eval-db        Created                                           0.3s
 ✔ Container eval-adminer   Created                      

docker compose ps   
NAME           IMAGE                COMMAND                  SERVICE   CREATED         STATUS         PORTS
eval-adminer   adminer:latest       "entrypoint.sh docke…"   adminer   7 minutes ago   Up 7 minutes   0.0.0.0:8080->8080/tcp, [::]:8080->8080/tcp
eval-db        postgres:15-alpine   "docker-entrypoint.s…"   db        7 minutes ago   Up 7 minutes   5432/tcp
```
**Questions :**
...
```bash
Un réseau Docker personnalisé permet d’isoler les services d’une application des autres conteneurs présents sur la machine.
Il offre une résolution DNS interne stable, ce qui permet aux services de communiquer simplement via leurs noms.
Il améliore la sécurité en limitant l’exposition des ports vers l’extérieur.
Enfin, il rend l’architecture plus claire et plus facile à maintenir, surtout avec plusieurs stacks ou environnements.
```
...
docker compose ps
NAME           IMAGE                COMMAND                  SERVICE   CREATED          STATUS                    PORTS
eval-adminer   adminer:latest       "entrypoint.sh docke…"   adminer   12 seconds ago   Up 11 seconds             0.0.0.0:8080->8080/tcp, [::]:8080->8080/tcp
eval-api       tp3-api              "docker-entrypoint.s…"   api       12 seconds ago   Up 11 seconds (healthy)   0.0.0.0:3000->3000/tcp, [::]:3000->3000/tcp
eval-db        postgres:15-alpine   "docker-entrypoint.s…"   db        12 seconds ago   Up 12 seconds             5432/tcp
eval-redis     redis:7-alpine       "docker-entrypoint.s…"   redis     12 seconds ago   Up 12 seconds             6379/tcp
...
[{"id":1,"name":"Alice","email":"alice@example.com","created_at":"2026-02-09T14:42:59.487Z"},{"id":2,"name":"Bob","email":"bob@example.com","created_at":"2026-02-09T14:42:59.487Z"},{"id":3,"name":"Charlie","email":"charlie@example.com","created_at":"2026-02-09T14:42:59.487Z"}]                                                                  

curl http://localhost:3000/api/stats
{"userCount":"3","timestamp":"2026-02-09T14:46:32.816Z","source":"cache"} 
```
...
```bash
On peut le vérifier en appelant plusieurs fois l’endpoint /api/stats : le premier appel renvoie une réponse provenant de la base de données (source: "database"), tandis que les suivants renvoient source: "cache".
On peut aussi se connecter directement à Redis avec redis-cli et utiliser GET stats et TTL stats pour constater que la clé existe et qu’elle expire automatiquement après quelques secondes.

➜  tp3 git:(main) ✗ docker exec -it eval-redis redis-cli
GET stats
TTL stats

127.0.0.1:6379> GET stats
"{\"userCount\":\"3\",\"timestamp\":\"2026-02-09T14:59:40.661Z\"}"
127.0.0.1:6379> TTL stats
(integer) 21
127.0.0.1:6379> 
```
- Que se passe-t-il si vous arrêtez le service Redis ?
```bash
Le cache ne fonctionne plus et l’API peut échouer sur les routes qui utilisent Redis (pas de try/catch).
curl http://localhost:3000/api/stats
...
--------+-------+-------+----------
 public | users | table | evaluser
(1 row)

evaldb=# SELECT * FROM users;
 id |  name   |        email        |         created_at         
----+---------+---------------------+----------------------------
  1 | Alice   | alice@example.com   | 2026-02-09 14:42:59.487165
  2 | Bob     | bob@example.com     | 2026-02-09 14:42:59.487165
  3 | Charlie | charlie@example.com | 2026-02-09 14:42:59.487165
(3 rows)
